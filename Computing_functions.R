...
update_tau_star_k_EM = function(db, mean_k, cov_k, kern, hp, pi_k)
{
  ...
  { 
    ...
    inv =  solve( kern_to_cov(db$Timestamp, kern, hp[1:2], hp[3]) + cov_k[[k]][paste0('X',t_i), paste0('X',t_i)] )

    mat_logL[c_k] =  dmvnorm(db %>% pull(Output) , mean_k[[k]] %>% filter(Timestamp %in% t_i) %>% pull(Output),
                               inv, log = T) ## classic gaussian loglikelihood
  } 
  ## We need to use the 'log-sum-exp' trick: exp(x - max(x)) / sum exp(x - max(x)) to remain numerically stable
