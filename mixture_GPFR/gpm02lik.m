function [fX, dfX] = gpm02lik(X, input, target, mu);
%
%mu: the mean of GP, having the same dimention as target
%
%
...
fX = 0.5*logdetQ + 0.5*(target-mu)'*invQ*(target-mu) + 0.5*n*log(2*pi);
