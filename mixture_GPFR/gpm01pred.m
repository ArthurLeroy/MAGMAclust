function [mu, S2, deriv, S2deriv, dummy] = gpm01pred(X, input, target, muin, test, mute);
...
if nargout == 1
  mu = mute + a'*(Q\(target-muin));     % don't compute invQ explicitly if we only need means
else
  invQ = inv(Q);
  invQt = invQ*(target-muin);
  mu = mute + a'*invQt;                                              % predicted means
